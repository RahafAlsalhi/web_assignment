...
app.get("/posts", async (req, res) => {
  try {
    const result = await db.query("SELECT * FROM posts ORDER BY id"); //to retun all the post in order
...
app.post("/posts", async (req, res) => {
  const { title, content, author } = req.body; //date and id auto updated
  try {
    const result = await db.query(
      "INSERT INTO posts(title,content,author) VALUES ($1,$2,$3) RETURNING *",
      [title, content, author]
    ); //order is important and return the data after insert
